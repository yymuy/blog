...
<div class="ibody">
    ...
    <article  class="bloglist">
        ...
        $_SESSION["articletitle"]=$result['title'];
        ?>
            <div class="bloglist">
                <div class="blogs">
                    ...
                    <p class="autor">
                        ...
                        <?php
                        $sql1 = "select * from tb_filecomment where fileid='$_GET[id]'";
        ...
        <div class="bloglist">
            <h2>
                <p>对博客主题<span>《<?php echo $_SESSION["articletitle"];?>》</span>的评价列表</p>
            </h2>
            <?php
            $rs=mysql_query("select * from tb_filecomment where fileid='$_GET[id]'");

                $perNumber=3; //每页显示的记录数
                $page=$_GET['page']; //获得当前的页面值
                $count=mysql_query("select count(*) from tb_filecomment where fileid='$_GET[id]'"); //获得记录总数
                $result=mysql_fetch_array($count);
                $totalNumber=$result[0];
                $totalPage=ceil($totalNumber/$perNumber); //计算出总页数

                $totalcount=mysql_num_rows(mysql_query("select * from tb_filecomment where fileid='$_GET[id]'"));
            if($totalcount < 1){
                echo "无相关评价";
            }
                if (!isset($page)) {
                ...
                $rs=mysql_query("select * from tb_filecomment where fileid='$_GET[id]' limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
            while($result = mysql_fetch_array($rs)) {
                ...
                <div class="bloglist">
                    <div class="blogs">
                        ...
                        <p class="autor">
                            <span>评价者：<?php echo $result['username']; ?></span>
                            <?php
                            ...
                            ?>
                                 ...
                                 $num=mysql_num_rows(mysql_query($sql1));
                                 ?>
                                <a href="huifu_de.php?fileid=<?php echo $result['id']?>">回复(<?php echo $num;?>)</a>&nbsp;&nbsp;&nbsp;

                                <?php
                                if($_SESSION[username] == $result['username']){
                                    $fileid="&fileid=".$_GET['id'];
                                    ?>
                                    <a href="del_pinglun.php?id=<?php echo $result['id'].$fileid;?>">删除</a>
                                    <?php
                                }
                                ?>
                            </span>
...
if($totalcount > 0) {
    ...
    <div class="pagey" >
        ...
        if ($page != 1) { //页数不等于1
            ?>
            <a href="articleList_de.php?page=<?php echo ($page - 1) . $id; ?>">上一页</a><!--显示上一页-->
            <?php
        }
        ...
        if ($page < $totalPage) { //如果page小于总页数,显示下一页链接
            ?>
            <a href="articleList_de.php?page=<?php echo ($page + 1) . $id; ?>">下一页</a><!--显示下一页-->
            <?php
        }
        ?>
...
<div class="container container_1000 m_center cf b_70 container_applay">
    ...
    <div class="cf f_left applay_left">
        <p class="title_applay b_30">我要评论</p>
        <form id="pinglunForm" action="pinglun.php?fileid=<?php echo $_GET['id'];?>" method="post">
            <table class="table_applay" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>
                        <label class="js_label">
                            ...
                            <textarea  class="input_applay input_applay_area js_input_applay" placeholder="请评价..." name="pinglun_text" required></textarea>
                        </label>
                    </td>
                </tr>
                </tbody>
                ...
                <tr>
                    <td>
                        ...
                        <input type="submit" value="提交" class="btn_submit" name="pinglun_btn"/>
                    </td>
                </tr>
                </tfoot>
            </table>
