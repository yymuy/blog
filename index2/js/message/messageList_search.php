...
<div class="ibody">
    ...
    <article  class="bloglist">
        ...
        if(isset($_GET['search_btn'])) {
            ...
            $count=mysql_query("select count(*) from tb_message where title like '%$title%'"); //获得记录总数
            $result=mysql_fetch_array($count);
            $totalNumber=$result[0];
            $totalPage=ceil($totalNumber/$perNumber); //计算出总页数

            $totalcount=mysql_num_rows(mysql_query("select * from tb_message where title like '%$title%'"));
            if($totalcount < 1){
                echo "无相关的留言信息";
            }
            if (!isset($page)) {
            ...
            $rs=mysql_query("select * from tb_message where title like '%$title%' limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
    ...
    <aside>
        ...
        <div class="tj_news">
            ...
            <ul>
                <?php
                $sql="select * from tb_message order by id desc limit 5";
                $rs = mysql_query($sql);
                while ($result = mysql_fetch_array($rs)) {
                    ?>
...
if($totalcount > 0){
    ...
    <div class="pagey">
        ...
        if ($page != 1) { //页数不等于1
            ?>
            <a href="messageList_search.php?page=<?php echo ($page - 1).$title;?>">上一页</a><!--显示上一页-->
            <?php
        }
        for ($i=1;$i<=$totalPage;$i++) {  //循环显示出页面
            ?>
            <a href="messageList_search.php?page=<?php echo $i.$title;?>"><?php echo $i;?></a>
            <?php
        }
        if ($page<$totalPage) { //如果page小于总页数,显示下一页链接
            ?>
            <a href="messageList_search.php?page=<?php echo ($page + 1).$title;?>">下一页</a><!--显示下一页-->
            <?php
        }
        ?>
...
<div class="container container_1000 m_center cf b_70 container_applay">
    <div class="cf f_left applay_left">
        ...
        <form id="messageForm" method="post" action="message.php">
                ...
                <tr>
                    <td>
                        <label class="js_label">
                            ...
                            <textarea  class="input_applay input_applay_area js_input_applay" placeholder="简单描述..." name="content"></textarea>
                        </label>
                    </td>
                </tr>
                </tbody>
