    ...
    <div class="main-wrap">
        ...
        <div class="result-wrap">
            <div class="crumb-wrap">
                ...
                $count=mysql_query("select count(*) from tb_filecomment"); //获得记录总数
                $result=mysql_fetch_array($count);
                $totalNumber=$result[0];
                $totalPage=ceil($totalNumber/$perNumber); //计算出总页数

                $totalcount=mysql_num_rows(mysql_query("select * from tb_filecomment"));
                if($totalcount < 1){
                ...
                $rs=mysql_query("select * from tb_filecomment limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
            ...
            if($totalcount > 0) {
                ...
                总记录数:<?php echo $totalcount; ?> 条</span>
                    ...
                    if ($page != 1) { //页数不等于1
                        ?>
                        <!--显示上一页-->
                        <a href="pinglunList.php?page=<?php echo $page - 1; ?>">上一页</a>
                        <?php
                    }
                    for ($i = 1; $i <= $totalPage; $i++) {  //循环显示出页面
                        ?>
                        <a href="pinglunList.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                        <?php
                    }
                    if ($page < $totalPage) { //如果page小于总页数,显示下一页链接
                        ?>
                        <!--显示下一页-->
                        <a href="pinglunList.php?page=<?php echo $page + 1; ?>">下一页</a>
                        <?php
                    }
                    ?>
                </div>
