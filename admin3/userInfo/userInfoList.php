...
<!--                                    <th class="tc" width="5%"></th>-->
                            ...
                            $count=mysql_query("select count(*) from tb_users"); //获得记录总数
                            $result=mysql_fetch_array($count);
                            $totalNumber=$result[0];
                            $totalPage=ceil($totalNumber/$perNumber); //计算出总页数

                            $totalcount=mysql_num_rows(mysql_query("select * from tb_users"));
                            ...
                            $rs=mysql_query("select * from tb_users limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
...
<!--                                <td class="tc"><input name="id[]" value="" type="checkbox"></td>-->
                                ...
                                ?>
                                <td>
                                    <a class="btn btn-primary btn2" href="del_user.php?regname=<?php echo $result['regname'];?>">删除</a>
                                </td>
                            </tr>
                                <?php
                            }
                        ...
                        <div class="list-page">
                            ...
                            if ($page != 1) { //页数不等于1
                                ?>
                                <a href="userInfoList.php?page=<?php echo $page - 1;?>">上一页</a>
                                <?php
                            }
                            for ($i=1;$i<=$totalPage;$i++) {  //循环显示出页面
                                ?>
                                <a href="userInfoList.php?page=<?php echo $i;?>"><?php echo $i;?></a>
                                <?php
                            }
                            if ($page<$totalPage) { //如果page小于总页数,显示下一页链接
                                ?>
                                <a href="userInfoList.php?page=<?php echo $page + 1;?>">下一页</a>
                                <?php
                            }
                            ?>
                        </div>
