    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i>
                ...
                <a class="crumb-name" href="/jscss/admin/design/">留言管理</a>
                <span class="crumb-step">&gt;</span><span>模糊查询</span>
            </div>
        </div>
        ...
        <div class="result-wrap">
            ...
            $count = mysql_query("select count(*) from tb_message where $tj like '%$key%'");//获得记录总数
            $result = mysql_fetch_array($count);
            $totalNumber = $result[0];
            $totalPage = ceil($totalNumber / $perNumber); //计算出总页数

            $totalcount = mysql_num_rows(mysql_query("select * from tb_message where $tj like '%$key%'"));
            if($totalcount < 1){
                echo "<span style='color: red'>暂无相关留言</span>";
            }
            if (!isset($page)) {
                $page = 1;
            } //如果没有值,则赋值1
            $startCount = ($page - 1) * $perNumber; //分页开始,根据此方法计算出开始的记录
            $rs = mysql_query("select * from tb_message where $tj like '%$key%' limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数

            while ($result = mysql_fetch_array($rs)) {
                ?>
                <div class="result-tab">
                    <p class="result-title"><?php echo $result['title']; ?></p>
                    <p style="margin-bottom:10px;width: 100px;text-indent: 2em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?php echo $result['content']; ?></p>
                    <p style="margin-bottom:10px;">
                        ...
                        ?>
                        <a href="messageList_de.php?messageid=<?php echo $result['id']?>" id="huifu">回复(<?php echo $num; ?>)</a>&nbsp;&nbsp;&nbsp;
            ...
            if($totalcount > 0) {
                ...
                总记录数:<?php echo $totalcount; ?> 条</span>
                    ...
                    if ($page != 1) { //页数不等于1
                        ?>
                        <!--显示上一页-->
                        <a href="messageList_sear.php?page=<?php echo ($page - 1) . $query; ?>">上一页</a>
                        <?php
                    }
                    for ($i = 1; $i <= $totalPage; $i++) {  //循环显示出页面
                        ?>
                        <a href="messageList_sear.php?page=<?php echo $i . $query; ?>"><?php echo $i; ?></a>
                        <?php
                    }
                    if ($page < $totalPage) { //如果page小于总页数,显示下一页链接
                        ?>
                        <!--显示下一页-->
                        <a href="messageList_sear.php?page=<?php echo ($page + 1) . $query; ?>">下一页</a>
                        <?php
                    }
                    ?>
                </div>
